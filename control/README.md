...
## Multi-Project extension (superbot-games)

This repo hosts **TWO** Projects: **game-mining** and **game-exploration**. The per-Project
control files are `control/status-mining.md` / `control/status-exploration.md` and
`control/inbox-mining.md` / `control/inbox-exploration.md`. The one-writer rule applies
**per file**: the manager writes both inboxes; each Project writes only ITS OWN status
file. Read everything above with `inbox.md` → *your* inbox file and `status.md` → *your*
status file.
